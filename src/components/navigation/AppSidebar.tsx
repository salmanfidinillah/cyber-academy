...
const navItems = [
  ...
  { label: "Jalur Belajar", route: "/learn/paths", icon: BookOpen },
  { label: "Simulasi", route: "/simulations", icon: ShieldAlert },
  { label: "AI Tutor", route: "/ai-tutor", icon: Bot },
  { label: "Progress", route: "/progress", icon: ChartNoAxesColumnIncreasing },
  { label: "Badge", route: "/badges", icon: Medal },
  { label: "Sertifikat", route: "/certificates", icon: Award },
  { label: "Profil", route: "/profile", icon: UserRound },
...
export const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  const displayName = currentUser?.displayName || "Pengguna";
  const avatarUrl =
  ...
  const itemClassName = (isActive: boolean) =>
    cn(
      ...
      "hover:border-black hover:bg-pastel-yellow/60 focus-visible:border-black focus-visible:ring-4 focus-visible:ring-pastel-yellow focus-visible:ring-offset-2",
      isActive && "border-black bg-pastel-mint text-black shadow-[3px_3px_0_0_#111111]",
      isCompact ? "mx-auto size-11 justify-center p-0" : "w-full gap-3 px-3 py-2",
    );

  return (
    <aside
      aria-label={isMobile ? "Navigasi aplikasi mobile" : "Navigasi utama"}
      className={cn(
        "left-0 top-0 z-40 flex h-dvh flex-col overflow-x-hidden border-r-4 border-brand-border bg-[#FFFDF8]",
        "transition-[width] duration-300 motion-reduce:transition-none",
        isMobile
          ? "relative w-full border-r-0"
          : "fixed",
        !isMobile &&
          (isCompact
    ...
    >
      ...
      >
        <Link
          ...
          aria-label="Cyber Academy — kembali ke dashboard"
          className={cn(
        ...
        >
          ...
          {!isCompact && (
            <span className="min-w-0 leading-tight">
              ...
              <span className="block truncate text-[10px] font-semibold text-gray-600">
                Belajar siber lebih aman
              </span>
            </span>
          )}
        </Link>
        ...
        {!isMobile && (
          <button
            type="button"
            onClick={onToggle}
            onMouseEnter={(event) => showTooltip(event, "Perluas sidebar", "tooltip-expand-sidebar")}
            onMouseLeave={hideTooltip}
            onFocus={(event) => showTooltip(event, "Perluas sidebar", "tooltip-expand-sidebar")}
            onBlur={hideTooltip}
            ...
            aria-describedby={isCompact && tooltip?.id === "tooltip-expand-sidebar" ? tooltip.id : undefined}
          >
            {isCompact ? (
      ...
      <nav className={cn("min-h-0 flex-1 space-y-1.5 overflow-y-auto overflow-x-hidden py-3", isCompact ? "px-2" : "px-3")}>
        {navItems.map((item) => {
          const isActive =
            ...
            (item.route === "/learn/paths" && currentPath.startsWith("/learn/")) ||
            (item.route === "/progress" && currentPath.startsWith("/progress/")) ||
            (item.route === "/settings" && currentPath.startsWith("/settings/"));
          ...
          return (
            <Link
              ...
              to={item.route}
              onClick={isMobile ? onToggle : undefined}
              onMouseEnter={(event) => showTooltip(event, item.label, `tooltip-${item.route}`)}
              onMouseLeave={hideTooltip}
              onFocus={(event) => showTooltip(event, item.label, `tooltip-${item.route}`)}
              onBlur={hideTooltip}
              className={itemClassName(isActive)}
              aria-current={isActive ? "page" : undefined}
              aria-label={isCompact ? item.label : undefined}
              aria-describedby={isCompact && tooltip?.id === `tooltip-${item.route}` ? tooltip.id : undefined}
            >
        ...
        {isAdmin && (
          <Link
            to="/admin"
            onClick={isMobile ? onToggle : undefined}
            onMouseEnter={(event) => showTooltip(event, "Panel Admin", "tooltip-admin-panel")}
            onMouseLeave={hideTooltip}
            onFocus={(event) => showTooltip(event, "Panel Admin", "tooltip-admin-panel")}
            onBlur={hideTooltip}
            className={cn(itemClassName(false), "text-blue-800 hover:bg-pastel-blue")}
            aria-label={isCompact ? "Panel Admin" : undefined}
            aria-describedby={isCompact && tooltip?.id === "tooltip-admin-panel" ? tooltip.id : undefined}
          >
            <UserCog className="size-5 shrink-0" aria-hidden="true" />
            {!isCompact && <span className="truncate text-sm">Panel Admin</span>}
          </Link>
        )}
      </nav>

      <div className={cn("shrink-0 space-y-2 border-t-4 border-brand-border", isCompact ? "p-2" : "p-3")}>
        <div
          className={cn(
            "flex min-w-0 items-center rounded-xl",
            isCompact ? "justify-center" : "gap-3 border-2 border-black bg-white p-2",
          )}
          onMouseEnter={(event) => showTooltip(event, displayName, "tooltip-user-profile")}
          onMouseLeave={hideTooltip}
        >
          <img
            src={avatarUrl}
        ...
        <button
          onClick={onLogout}
          onMouseEnter={(event) => showTooltip(event, "Keluar", "tooltip-logout")}
          onMouseLeave={hideTooltip}
          onFocus={(event) => showTooltip(event, "Keluar", "tooltip-logout")}
          onBlur={hideTooltip}
          className={cn(
          ...
          aria-describedby={isCompact && tooltip?.id === "tooltip-logout" ? tooltip.id : undefined}
        >
